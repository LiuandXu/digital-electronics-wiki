...
## 例题7：ROM实现组合逻辑函数（2023 A卷 综合三）

**题目**：下图给出了一个ROM阵列结构，地址输入为A、B、C，输出为X、Y。写出X和Y的逻辑表达式，说明电路功能。

<figure markdown>
  ![ROM阵列结构图](images/2023A_p3_1.jpeg){ width="450" }
  <figcaption>图5：ROM与阵列+或阵列结构（地址A/B/C，输出X/Y）</figcaption>
</figure>
...
ROM左侧为与阵列（3线-8线译码器），将输入A、B、C译为8条字线 \(W_0 \sim W_7\)（对应最小项 \(m_0 \sim m_7\)）。

右侧为或阵列，字线与输出位线的交叉点表示存储"1"。
...
X = \overline{A}\,\overline{C} + \overline{B}\,\overline{C} + ABC

\]

\[

Y = \overline{A}\,BC + AB\,C = BC(\overline{A} + A) = BC

\]

!!! tip "ROM实现组合逻辑"
    ROM的本质是一个查找表（LUT）：地址作为输入，存储内容作为输出。n位地址ROM可实现任意n变量组合逻辑函数。写入数据对应真值表中输出为1的最小项。

---

## 例题8：74LS138译码器功能表（2020 A卷 简答题 附图）

**题目**：下图给出3线-8线译码器74LS138的功能表和逻辑符号。

<figure markdown>
  ![74LS138功能表](images/2020A_p3_1.png){ width="500" }
  <figcaption>图6：74LS138 3线-8线译码器功能表</figcaption>
</figure>

**解答**：

74LS138关键特性：
- 3个地址输入 \(A_2 A_1 A_0\)，8个输出 \(\overline{Y_0} \sim \overline{Y_7}\)（低电平有效）
- 3个使能端：\(G_1\)（高有效）、\(\overline{G_{2A}}\)、\(\overline{G_{2B}}\)（低有效）
- 只有当 \(G_1=1\) 且 \(\overline{G_{2A}}=\overline{G_{2B}}=0\) 时才译码

---

## 例题9：8选1数据选择器功能表（2020 A卷 综合一 附图）

**题目**：下图给出8选1数据选择器的功能表。

<figure markdown>
  ![8选1数据选择器功能表](images/2020A_p7_1.png){ width="450" }
  <figcaption>图7：8选1数据选择器功能表</figcaption>
</figure>

**解答**：

8选1数据选择器：
- 3个地址输入 \(A_2 A_1 A_0\)，8个数据输入 \(D_0 \sim D_7\)
- 输出 \(Y = \sum_{i=0}^{7} m_i D_i\)，其中 \(m_i\) 为地址对应最小项
- 利用数据选择器可实现任意组合逻辑函数：将函数变量接地址端，数据端按真值表接0或1

---

## 例题10：74160+74LS151序列信号发生器（2023 A卷 综合四）

**题目**：下图由74160十进制计数器和74LS151 8选1数据选择器构成序列信号发生器，分析输出序列。

<figure markdown>
  ![74160+74LS151序列发生器](images/2023A_p4_1.jpeg){ width="500" }
  <figcaption>图8：74160计数器+74LS151数据选择器构成的序列发生器</figcaption>
</figure>

**解答**：

**工作原理**：
1. 74160构成模10计数器（0~9循环），输出 \(Q_D Q_C Q_B Q_A\) 作为74LS151的地址
2. 74LS151根据地址选择 \(D_0 \sim D_7\) 中的一路输出
3. 计数器每变化一个状态，数据选择器输出对应位，形成序列

**分析步骤**：
- 74160计数序列：0000→0001→...→1001→0000
- 74LS151地址 \(A_2 A_1 A_0 = Q_C Q_B Q_A\)
- 根据数据输入端连接（高电平或低电平）确定输出序列

!!! note "序列发生器两种实现方式"
    1. **计数器+数据选择器**（本例）：计数器产生地址，MUX选择数据
    2. **计数器+ROM**：计数器产生地址，ROM存储数据，DAC转换为模拟量

---

## 例题11：ROM阵列结构分析（2023 A卷 综合三 附图）

**题目**：下图给出ROM的阵列结构。

<figure markdown>
  ![ROM阵列结构](images/2023A_p2_1.png){ width="500" }
  <figcaption>图9：ROM与阵列-或阵列结构图</figcaption>
</figure>

**解答**：

ROM阵列由两部分组成：
- **与阵列**（左侧）：实现地址译码，产生字线 \(W_i = m_i\)
- **或阵列**（右侧）：实现数据存储，位线 \(Y_j = \sum\) 对应交叉点的字线

交叉点处有圆点表示连接（存储"1"），无圆点表示不连接（存储"0"）。
