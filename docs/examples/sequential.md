...
<figure markdown>
  ![74161功能表](images/2022B_p5_1.png){ width="450" }
  <figcaption>图2：74161功能表（CLR/LD/P/T/CP与工作状态对应关系）</figcaption>
</figure>
...
## 例题5：T/D触发器混合时序电路分析（2022 B卷 综合三）

**题目**：分析下图所示时序逻辑电路，写出驱动方程和状态方程，列出状态转换表，说明电路功能。

<figure markdown>
  ![T/D触发器时序电路](images/2022B_p7_1.png){ width="500" }
  <figcaption>图3：T触发器+D触发器构成的同步时序电路</figcaption>
</figure>

**电路结构**（由图可知）：

- FF1：T触发器，\(T_1 = \overline{Q_3}\)，下降沿触发
- FF2：D触发器，\(D_2 = Q_1\)，下降沿触发
- FF3：D触发器，\(D_3 = Q_2\)，下降沿触发
- 输出：\(Z = Q_1 \cdot Q_3\)
- 三触发器共用同一时钟CP
...
\[
T_1 = \overline{Q_3^n}, \quad D_2 = Q_1^n, \quad D_3 = Q_2^n
\]

**步骤二：状态方程**

T触发器特征方程：\(Q^{n+1} = T \oplus Q^n\)

\[
Q_1^{n+1} = \overline{Q_3^n} \oplus Q_1^n
\]
...
该电路为 **同步五进制计数器**（或序列信号发生器），具有自启动能力。输出Z在状态111时为1，可作为进位输出。

!!! note "T/D混合触发器分析"
    T触发器和D触发器混合使用时，注意各自的特征方程不同：
    - T触发器：\(Q^{n+1} = T \oplus Q^n\)（T=0保持，T=1翻转）
    - D触发器：\(Q^{n+1} = D\)（输出跟随输入）

---

## 例题6：74161逻辑符号与功能（2022 A卷 综合二 附图）

**题目**：下图给出74161的逻辑符号，结合功能表分析各引脚作用。

<figure markdown>
  ![74161逻辑符号](images/2022A_p5_1.png){ width="350" }
  <figcaption>图4：74161 4位同步二进制计数器逻辑符号</figcaption>
</figure>
...
| \(\overline{LD}\) | 同步置数 | 低电平有效，下一CP置数 |
| \(P, T\) | 计数使能 | 均为高电平时允许计数 |
| CP | 时钟输入 | 上升沿触发 |
| \(D \sim A\) | 并行数据输入 | 置数时载入 |
| \(Q_D \sim Q_A\) | 计数输出 | 当前计数值 |
| \(RCO\) | 进位输出 | \(Q_D Q_C Q_B Q_A \cdot T = 1\)时输出高 |

!!! tip "74161 vs 74163"
    74161和74163的唯一区别：74161是**异步清零**，74163是**同步清零**。设计任意进制计数器时需注意此差异。

---

## 例题7：JK触发器时序电路分析（2023 B卷 综合三）

**题目**：分析下图所示同步时序电路，写出驱动方程、状态方程，列出状态转换表，说明电路功能。

<figure markdown>
  ![JK触发器时序电路](images/2023B_p6_1.png){ width="500" }
  <figcaption>图5：JK触发器构成的同步时序电路</figcaption>
</figure>
...
\[
Q_2^{n+1} = (Q_1^n Q_0^n) \oplus Q_2^n
\]
...
该电路为 **同步三进制... 实际为同步八进制（模8）加法计数器**，所有8个状态构成有效循环，无需检查自启动。

!!! note "同步二进制计数器通用公式"
    n位同步二进制加法计数器中，第i个触发器的驱动方程为：
    
    \[
    J_i = K_i = Q_0^n \cdot Q_1^n \cdots Q_{i-1}^n
    \]
...
## 例题8：T4161级联+D触发器分频电路（2022 A卷 综合二）

**题目**：下图由两片T4161同步计数器级联，末级接D触发器构成分频电路。输入时钟 \(f_{CP} = 20\text{kHz}\)，求输出Y的频率。

<figure markdown>
  ![T4161级联+D触发器分频电路](images/2022A_p2_1.jpeg){ width="550" }
  <figcaption>图6：两片T4161同步级联 + D触发器二分频</figcaption>
</figure>
...
两片T4161同步级联（共同时钟CP），第一片进位输出C控制第二片的使能端 \(S_2\)，构成8位二进制计数器。

**步骤二：分析清零/置数逻辑**

两片的 \(\overline{LD}\) 和 \(\overline{R_D}\) 由与门控制，当计数到特定状态时置零，构成任意进制计数器。

**步骤三：分析D触发器**

末级D触发器 \(D = \overline{Q}\)，构成T'触发器（二分频），对计数器进位信号再除2。
